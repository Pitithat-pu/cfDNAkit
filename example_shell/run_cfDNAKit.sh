...
binsize="100"
config_file=${cfDNAkit_dir}/config_files/set_parameters_no_trimming_${binsize}KB_PON.R
runtime_variable_filename="runtime_variables_${binsize}kb.R"
...
# sample_ids="K34R-1UWART K34R-23MWWD K34R-58ULPU K34R-6GMRNB"
# sample_ids="K34R-PKN8UU"
...
for sample_id in ${sample_ids};do
	...
	for bamfile in ${alignment_dir}/*_${sample_id}_merged.mdup.bam; do
		...
		COVERAGE_ZSCORE_CMD="${load_R_cmd}; Rscript ${cfDNAkit_dir}/coverage_zscore/coverage_zscore.R ${runtime_variables_file} ${config_file}"
...
# echo "Submit EXTRACT_DENSITY_INFO process : job id ${EXTRACT_DENSITY_INFO_job_id}"
		...
		COVERAGE_ZSCORE_job_id=$(echo "${COVERAGE_ZSCORE_CMD}" | bsub -J ${sample_id}_COVERAGE_ZSCORE -W 10:00 -M 30GB -w "done(${EXTRACT_BAM_INFO_job_id})" | awk '/is submitted/{print substr($2, 2, length($2)-2);}')
		echo "Submit COVERAGE_ZSCORE process : job id ${COVERAGE_ZSCORE_job_id}"
